...
public class ProjectController {
	...
	@PostMapping("/user/success-project" )
	@ResponseBody
	public String falseProject(@RequestBody Map<String,Integer > userid) {
		System.out.println(userid.get("user_id"));
		List<Project> list  = projectService.findProjectOfUserByUserId(userid.get("user_id"));
		int a=0;
	...
	@PostMapping("/user/running-project" )
	@ResponseBody
	public String runningProject(@RequestBody Map<String,Integer > userid) {
		System.out.println(userid.get("user_id"));
		List<Project> list = projectService.findProjectOfUserByUserId(userid.get("user_id"));
		int  b=0;
		for (Project project : list) {
			if(project.getStatus().equals("running")) {
				b++;
			}
		}
		return "{\"number\":" +b+ "}"; 
	...
	@PostMapping("/user/failed-project" )
	@ResponseBody
	public String trueProject(@RequestBody Map<String,Integer> userid) {
		List<Project> list = projectService.findProjectOfUserByUserId(userid.get("user_id"));
		int  b=0;
		for (Project project : list) {
			if(project.getStatus().equals("true")) {
				b++;
			}
		}
		return "{\"number\":" +b+ "}"; 
